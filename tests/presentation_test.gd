...
func _run() -> void:
	...
	hunter_presentation.animate_action(&"light_attack", &"active", 0.25, 0.0, 0.0, 0, Hunter.LIGHT_ACTION, Hunter.CHARGE_ACTIONS[1].charge_threshold, Hunter.CHARGE_CANCEL_DURATION, Hunter.DODGE_ACTION.duration())
	_expect(hunter_presentation.current_animation == &"light_attack", "Light attack logic samples the matching authored action")
	hunter_presentation.animate_action(&"charging", &"windup", 0.0, 0.15, 0.0, 0, null, Hunter.CHARGE_ACTIONS[1].charge_threshold, Hunter.CHARGE_CANCEL_DURATION, Hunter.DODGE_ACTION.duration())
	_expect(hunter_presentation.current_animation == &"charge_enter", "Early charge samples the authored entry action")
	hunter_presentation.animate_action(&"charging", &"windup", 0.0, Hunter.CHARGE_ACTIONS[0].charge_threshold, 0.0, 1, null, Hunter.CHARGE_ACTIONS[1].charge_threshold, Hunter.CHARGE_CANCEL_DURATION, Hunter.DODGE_ACTION.duration())
	_expect(hunter_presentation.current_animation == &"charge_hold", "Charge tier I selects the authored hold action")
	hunter_presentation.animate_action(&"charge_release", &"active", 0.2, 0.0, Hunter.CHARGE_ACTIONS[1].charge_threshold, 2, Hunter.CHARGE_ACTIONS[1], Hunter.CHARGE_ACTIONS[1].charge_threshold, Hunter.CHARGE_CANCEL_DURATION, Hunter.DODGE_ACTION.duration())
	_expect(hunter_presentation.current_animation == &"charge_release_2", "Full charge selects the tier-II release action")
	hunter_presentation.animate_action(&"dodging", &"invulnerable", 0.1, 0.0, 0.0, 0, Hunter.DODGE_ACTION, Hunter.CHARGE_ACTIONS[1].charge_threshold, Hunter.CHARGE_CANCEL_DURATION, Hunter.DODGE_ACTION.duration())
