...
func _run() -> void:
	...
	_expect(monster_presentation.monster_model != null and monster_presentation.monster_model.scene_file_path.ends_with("monster.glb"), "Monster uses the authored horned field beast GLB")
	_expect(_find_type(monster_presentation.monster_model, Skeleton3D) != null, "Monster presentation retains the imported quadruped skeleton")
	_expect(_find_type(monster_presentation.monster_model, AnimationPlayer) != null, "Monster presentation retains the imported action library")
	_expect(monster_presentation.current_animation == &"idle", "Monster presentation initializes in its authored idle pose")
	_expect(monster_presentation.visuals.find_child("BarrelBody", true, false) != null and monster_presentation.visuals.find_child("HornLOuter", true, false) != null, "Authored body and outward horns replace the programmatic placeholder")
	...
	monster_presentation.animate_walk(0.016, monster.move_speed, monster.move_speed, false)
	_expect(monster_presentation.current_animation == &"run", "Monster chase speed selects the authored run loop")
	monster_presentation.animate_attack(FieldMonster.SWEEP_ATTACK, FieldMonster.PHASE_ACTIVE, FieldMonster.SWEEP_ATTACK.windup + 0.1, FieldMonster.CHARGE_CRASH_RECOVERY)
	_expect(monster_presentation.current_animation == &"sweep", "Horn sweep logic samples the authored sweep action")
	monster_presentation.animate_attack(FieldMonster.POUNCE_ATTACK, FieldMonster.PHASE_ACTIVE, FieldMonster.POUNCE_ATTACK.windup + FieldMonster.POUNCE_ATTACK.active * 0.5, FieldMonster.CHARGE_CRASH_RECOVERY)
	_expect(monster_presentation.current_animation == &"pounce" and monster_presentation.is_airborne(), "Pounce logic samples its airborne authored pose")
	monster_presentation.animate_attack(FieldMonster.CHARGE_ATTACK, FieldMonster.PHASE_WINDUP, FieldMonster.CHARGE_ATTACK.windup * 0.5, FieldMonster.CHARGE_CRASH_RECOVERY)
	_expect(monster_presentation.current_animation == &"charge_windup" and monster_presentation.horn_glowing(), "Charge windup lowers and highlights the authored horns")
	monster_presentation.animate_attack(FieldMonster.CHARGE_ATTACK, FieldMonster.PHASE_STUNNED, FieldMonster.CHARGE_ATTACK.windup + FieldMonster.CHARGE_ATTACK.active + 0.5, FieldMonster.CHARGE_CRASH_RECOVERY)
	_expect(monster_presentation.current_animation == &"crash_stunned" and monster_presentation.is_knocked_down(), "Charge crash selects the authored side-down stun")
	monster_presentation.update_feedback(FieldMonster.STATE_IDLE, &"ready", 0.12)
	monster_presentation.animate_walk(0.016, monster.move_speed, monster.move_speed, false)
	_expect(monster_presentation.current_animation == &"hit", "Accepted monster damage keeps the authored hit reaction visible")
	monster_presentation.play_defeated()
	_expect(monster_presentation.current_animation == &"defeated", "Monster defeat selects and holds the authored down pose")
	monster_presentation.reset_pose()
	_expect(monster_presentation.current_animation == &"idle" and not monster_presentation.horn_glowing(), "Monster reset clears animation residue and restores idle")
