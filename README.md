...
当前阶段：**M3 狩猎闭环**。M3.3 已为怪物接入共用攻击状态机与第一招横扫，正在进行浏览器人工验收。多种普攻动作暂缓到后续迭代；角色、大剑与怪物仍为可替换的程序化占位外观。
...
右后方的场地怪物会追近玩家并使用横扫：琥珀色前摇会继续瞄准，转为红色时方向已经锁定，红色提示短暂出现后双角物理探针才开始造成伤害，蓝灰色收招阶段可以安全反击。横扫可走出角的实际扫掠路径或用翻滚无敌帧规避，每次攻击最多扣血一次；怪物身体接触本身不造成伤害。玩家受击时会闪红并短促后仰震动。玩家可用普攻或蓄力击杀怪物，R 会恢复双方位置与生命。中央训练靶仍可用于练习。普攻不消耗体力；蓄力期间逐渐消耗体力，满蓄力累计消耗 30，提前释放或取消只消耗已经扣除的部分；开始翻滚扣 24。开始蓄力仍需至少 30 体力。攻击期间不可翻滚取消，蓄力期间可以用翻滚放弃蓄力。
