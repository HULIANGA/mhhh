...
当前阶段：**M3 狩猎闭环**。M3.4 已在共用怪物攻击状态机上加入中距离扑击，正在进行浏览器人工验收。多种普攻动作暂缓到后续迭代；角色、大剑与怪物仍为可替换的程序化占位外观。
...
右后方的场地怪物会追近玩家并按距离使用两种攻击：贴近时横扫，中距离时先俯身蓄势、锁定方向，再沿直线高速扑出。横扫使用随双角运动的物理探针，扑击使用覆盖每帧移动路径的连续物理探针；两者都可通过离开锁定路径或翻滚规避，每次攻击最多扣血一次。蓝灰色收招阶段可以安全反击，怪物身体接触本身不造成伤害。玩家受击时会闪红并短促后仰震动。玩家可用普攻或蓄力击杀怪物，R 会恢复双方位置与生命。中央训练靶仍可用于练习。普攻不消耗体力；蓄力期间逐渐消耗体力，满蓄力累计消耗 30，提前释放或取消只消耗已经扣除的部分；开始翻滚扣 24。开始蓄力仍需至少 30 体力。攻击期间不可翻滚取消，蓄力期间可以用翻滚放弃蓄力。
