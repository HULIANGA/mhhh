...
当前阶段：**M3 狩猎闭环**。M3.6 已加入可复现的带权重招式选择、连续同招限制和重复招冷却调节，正在进行浏览器人工验收。多种普攻动作暂缓到后续迭代；角色、大剑与怪物仍为可替换的程序化占位外观。
