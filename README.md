...
当前阶段：**M3 狩猎闭环**。M3.2 已加入具备独立生命、受击、死亡和直接追击状态的程序化占位怪物，正在进行浏览器人工验收；怪物攻击从 M3.3 开始逐项接入。多种普攻动作暂缓到后续迭代；角色、大剑与怪物仍为可替换的程序化占位外观。
...
右后方的场地怪物会发现并追近玩家，在金色攻击距离环外停步；M3.2 中它尚未攻击，身体接触也不会造成伤害。玩家可用普攻或蓄力击杀它，R 会恢复双方位置与生命。中央训练靶仍可用于练习。普攻不消耗体力；蓄力期间逐渐消耗体力，满蓄力累计消耗 30，提前释放或取消只消耗已经扣除的部分；开始翻滚扣 24。开始蓄力仍需至少 30 体力。攻击期间不可翻滚取消，蓄力期间可以用翻滚放弃蓄力。每次攻击对同一目标只结算一次伤害。
...
- `scripts/monster.gd`、`scenes/monster.tscn`：怪物生命、受击、死亡、追击与程序化占位外观。
