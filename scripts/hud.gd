...
func _ready() -> void:
	...
	objective = _label(content, "Read the warning. Evade the sweep. Punish recovery.", 19, INK)
...
func _build_overlay() -> void:
	...
	_label(box, "PROTOTYPE  M3.3    /    Horn sweep", 12, MUTED)

func show_menu(first_time: bool) -> void:
	overlay.show()
	title.text = "A heavy blade rewards\ndeliberate hands." if first_time else "Take a breath."
	description.text = "Amber warns of a sweep; red marks its active arc, and blue-grey recovery is your opening.\nStep outside the arc or roll through it, then answer with the heavy blade." if first_time else "The field is paused. Your action is frozen in place.\nResume when you are ready to commit."
...
func set_target_health(current: int, maximum: int) -> void:
	...
	objective.text = "Beast down. Press R to reset the pursuit." if current <= 0 else "Read the warning. Evade the sweep. Punish recovery."
